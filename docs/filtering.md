...
| ai-patterns | Mixed | Mixed | Top-level bot observability metrics include all activity with observable commit metadata, but `humanReviewBurden` always excludes traditional bot-authored PRs, PRs whose AI classification is not observable at the cutoff, and bot reviews from the comparison metrics |
...
For `ai-patterns`, this split is intentional: bot observability (`botReviewers`, `botReviewPercentage`, `aiCoAuthoredPRs`, `totalPRs`) uses the full dataset, while `humanReviewBurden` uses a comparison cohort that excludes traditional bot-authored PRs regardless of `include-bots` and excludes PRs whose AI classification is not observable at the cutoff.
